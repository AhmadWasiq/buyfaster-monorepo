...
export function Orb({
  ...
  return (
    <div className={className ?? "relative h-full w-full"} style={{ width: '100%', height: '100%' }}>
      <Canvas
        resize={{ debounce: Math.min(resizeDebounce, 50), scroll: false }}
        dpr={[1, 2]}
        gl={{
          alpha: true,
          antialias: true,
...
void main() {
    ...
    vec3 color1 = vec3(0.0, 0.0, 0.0);             // shadow - stays black
    vec3 color2 = vec3(0.047, 0.478, 0.553);       // #0C7A8D (main color)
    vec3 color3 = vec3(0.031, 0.361, 0.415);       // #085C6A (darker highlight)
    vec3 color4 = vec3(1.0, 1.0, 1.0);             // white (barely visible)
